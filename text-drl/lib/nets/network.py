...
class Network(object):
  ...
  def _roi_pool_layer(self, bootom, rois, name):
    with tf.variable_scope(name) as scope:
      return tf.image.roi_pooling(bootom, rois,
                                  pooled_height=cfg.POOLING_SIZE_H,
                                  pooled_width=cfg.POOLING_SIZE_W,
  ...
  def _crop_pool_layer(self, bottom, rois, name):
    with tf.variable_scope(name) as scope:
      ...
      pre_pool_size_H = cfg.POOLING_SIZE_H * 2
      pre_pool_size_W = cfg.POOLING_SIZE_W * 2
      crops = tf.image.crop_and_resize(bottom, bboxes, tf.to_int32(batch_ids),
                                       [pre_pool_size_H, pre_pool_size_W],
                                       name="crops")
